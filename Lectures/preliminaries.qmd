...
[**Download R Code for this Lecture**](code/preliminaries.R){ .btn .btn-primary }
...
- H₀: The mean pain score is the same for both groups.
- Hₐ: The mean pain score differs between groups.
...
> **💡 R Tip**: In R, categorical variables should be stored as **factors**. Use `factor()` to convert character variables to factors, especially for ordinal data where you can specify the order.
...
> **💡 R Tip**: In R, p-values are typically found in the output of statistical tests. Look for columns labeled `Pr(>|t|)`, `p-value`, or similar. A common threshold is p < 0.05 for statistical significance.
...
> **💡 R Tip**: Common effect sizes (Cohen's d): small = 0.2, medium = 0.5, large = 0.8. The `pwr` package has functions for many test types: `pwr.t.test()`, `pwr.anova.test()`, `pwr.chisq.test()`, etc.
...
> **💡 R Tip**: Keep all files for a project in one folder. This makes it easier to manage your data, scripts, and output.
...
> **💡 R Tip**: Use `str(my_data)` to see the structure of your data frame. This shows variable names, types, and first few values - very helpful for understanding your data!
